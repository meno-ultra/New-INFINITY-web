...
const { PUBLIC_ROOT, publicPath } = require("./config/paths");
...
app.get("/robots.txt", (req, res) => {
    const filePath = publicPath("robots.txt");
...
app.get("/sitemap.xml", (req, res) => {
    const filePath = publicPath("sitemap.xml");
    if (!fs.existsSync(filePath)) {
        res.status(404);
        res.setHeader("Content-Type", "text/plain; charset=utf-8");
...
app.use(async (req, _res, next) => {
    try {
        ...
        const unchanged = prev
            && prev.id === nextSessionUser.id
            && prev.email === nextSessionUser.email
            && prev.name === nextSessionUser.name
            && prev.role === nextSessionUser.role
            && prev.accountType === nextSessionUser.accountType;
        if (!unchanged) {
...
function normalizeSpecSections(rawSections, rawSpecs, descriptionEn = "", descriptionAr = "") {
    ...
    if (Array.isArray(rawSections)) {
        ...
        rawSections.split("\n").forEach((line) => {
            ...
            if (!currentTitle) currentTitle = "1. Ø§Ù„Ù…ÙˆØ§ØµÙØ§Øª Ø§Ù„ÙÙ†ÙŠØ©";
    ...
    if (flatSpecs.length) {
        return [{ title: "1. Ø§Ù„Ù…ÙˆØ§ØµÙØ§Øª Ø§Ù„ÙÙ†ÙŠØ©", items: flatSpecs }];
    }
    return [];
}
...
async function deletePaymentReceiptAsset(orderId, receiptImage) {
    ...
    if (String(receiptImage).startsWith("assets/orders/receipts/")) {
        const abs = path.join(PUBLIC_ROOT, receiptImage);
        try {
...
async function deleteProductImageAsset(productId, imageUrl) {
    ...
    if (String(imageUrl).startsWith("assets/products/")) {
        const abs = path.join(PUBLIC_ROOT, imageUrl);
        try {
            if (fs.existsSync(abs)) fs.unlinkSync(abs);
        } catch (err) {
...
async function start() {
    try {
        ...
        console.error("- In MongoDB Atlas: Network Access â†’ add your IP (or 0.0.0.0/0 for testing)");
...
app.patch("/api/dashboard/products/:productId/stock", requireRole(["employee", "manager", "primary"]), async (req, res) => {
    try {
        ...
        if (specSections !== undefined) {
            ...
            update.specs = flat;
            update.specSections = flat.length ? [{ title: "1. Ø§Ù„Ù…ÙˆØ§ØµÙØ§Øª Ø§Ù„ÙÙ†ÙŠØ©", items: flat }] : [];
        }
...
app.use("/api/support", createSupportRouter({ User, requireAuth }));
...
module.exports = { app, start, PORT, sessionMiddleware };
